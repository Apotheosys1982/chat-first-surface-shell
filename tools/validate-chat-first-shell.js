...
function validateTarget(target, checks) {
  ...
  push(checks, target.id, "stream_touch_pressure_controls_chrome_motion", /function\s+canHandleChromeGesture[\s\S]*?return[\s\S]*?\.assistant-composer[\s\S]*?side-tray[\s\S]*?;[\s\S]*?}/i.test(script) && !/function\s+canHandleChromeGesture[\s\S]*?\[data-message-stream\][\s\S]*?}/i.test(script) && /touchmove[\s\S]*gestureOwnsChrome\(-fingerDelta,\s*event\.target\)/i.test(script), "touch pressure inside the message stream must reveal/collapse chrome immediately");
  ...
  push(checks, target.id, "project_state_artifacts_registered", /"project-state-dashboard"\s*:\s*{[\s\S]*Project state dashboard/i.test(script) && /"artifact-directory"\s*:\s*{[\s\S]*Artifact directory/i.test(script) && /"activity-log"\s*:\s*{[\s\S]*Recent hardening log/i.test(script), "dashboard, artifact directory, and activity log artifacts");
  ...
  push(checks, target.id, "spreadsheet_local_workbook_runtime_exists", /SPREADSHEET_STORE_KEY/i.test(script) && /loadSpreadsheetStore/i.test(script) && /saveSpreadsheetStore/i.test(script) && /createBlankSpreadsheetWorkbook/i.test(script), "browser-local editable spreadsheet store");
  push(checks, target.id, "spreadsheet_create_command_exists", /handleSpreadsheetAssistantCommand/i.test(script) && /isCreateSpreadsheetCommand/i.test(script) && /data-spreadsheet-create/i.test(script), "assistant and UI can create blank local spreadsheet artifacts");
  push(checks, target.id, "spreadsheet_export_and_validation_exist", /function\s+exportWorkbookCsv\s*\(/i.test(script) && /workbookToCsv/i.test(script) && /spreadsheetValidationSummary/i.test(script) && /spreadsheetSourceStatusHtml/i.test(script), "spreadsheet export/source/validate controls are wired");
  push(checks, target.id, "spreadsheet_upload_does_not_auto_approve", /createSpreadsheetFromDelimitedText/i.test(script) && /origin:\s*["']userUpload["'][\s\S]*sourceStatus:\s*["']Uncertain["'][\s\S]*ingestionStatus:\s*["']Extracted["'][\s\S]*canAnswerFrom:\s*false/i.test(script), "parsed spreadsheet uploads render but remain unapproved");
