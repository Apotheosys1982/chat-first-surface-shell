...
function validatesChatWorkspace(publicDir, css, script, checks) {
  ...
  push(checks, publicDir, "chat_scroll_position_cannot_auto_hide_chrome", !/top\s*>\s*CHROME_SCROLL_THRESHOLD/i.test(script) && !/!\s*shell\?\.classList\.contains\(["']chrome-receded["']\)\s*&&\s*top/i.test(script), "stream position must not hide chrome after upward pressure reveals it");
  ...
  push(checks, publicDir, "chat_stream_touch_pressure_controls_chrome", /function\s+canHandleChromeGesture[\s\S]*?return[\s\S]*?\.assistant-composer[\s\S]*?side-tray[\s\S]*?;[\s\S]*?}/i.test(script) && !/function\s+canHandleChromeGesture[\s\S]*?\[data-message-stream\][\s\S]*?}/i.test(script) && /touchmove[\s\S]*gestureOwnsChrome\(-fingerDelta,\s*event\.target\)/i.test(script), "message stream touch pressure must reveal/collapse chrome immediately");
}
